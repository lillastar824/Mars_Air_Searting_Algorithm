    ...
    row.forEach((seat, si)=>{
      if (seat && (i < n)) {
        if (n == 1) {
          ...
          availability[ri][si] = 0;
          i++;
        } else if ((n == 4) && (si > 1) && (availableSeats(row)+i >= 4)) {
          assignedSeats.push(seats[ri][si]);
          availability[ri][si] = 0;
          i++;
        } else if ((n > 4) && (availableSeats(row)+i == rowCapacity)) {
          if (availableSeats(row)+i > 4) {
            assignedSeats.push(seats[ri][si]);
            availability[ri][si] = 0;
            i++;
          }
        } else if (n > rowCapacity) {
          if (availableSeats(row)+i >= rowCapacity) {
            assignedSeats.push(seats[ri][si]);
            availability[ri][si] = 0;
            i++;
          }
